defmodule Discuss.VotesTest do
  ...
  describe "option" do
    test "create_option/1 creates an option on a poll", %{user: user} do
      with {:ok, poll} = Votes.create_poll(%{title: "Simple Poll", user_id: user.id}),
           {:ok, option} =
             Votes.create_option(%{title: "Simple Choice", votes: 0, poll_id: poll.id}),
