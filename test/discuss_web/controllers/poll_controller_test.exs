defmodule DiscussWeb.PollControllerTest do
  ...
  test "GET /polls", %{conn: conn, user: user} do
    {:ok, poll} =
      Discuss.Votes.create_poll_with_options(%{title: "Poll 1", user_id: user.id}, [
        "Choice 1",
        "Choice 2",
        "Choice 3"
      ])
  ...
  test "GET /options/:id/vote", %{conn: conn, poll: poll} do
    ...
    after_option = Discuss.Repo.get!(Discuss.Votes.Option, option.id)
